<template>
  <section id="faq">
    <!-- seach banner  -->
    <v-card flat class="faq-bg d-flex align-center justify-center text-center mb-9">
      <v-card-text>
        <p class="faq-title text-2xl font-weight-semibold primary--text mb-2">Hello, how can we help?</p>
    ...
    <v-card-text class="question-section">
      <v-row>
        <v-col v-for="item in filterFaqData" :key="item.title" md="10" offset-md="2" cols="12" class="mx-auto mb-8">
          <div class="faq-title d-flex align-center">
            <v-avatar color="secondary" rounded size="40" class="v-avatar-light-bg">
              <v-icon size="24" class="text--primary">
                {{ item.icon }}
              </v-icon>
            </v-avatar>
          ...
          <div class="faq-question-answers mt-6">
            <v-expansion-panels>
              <v-expansion-panel v-for="data in item.qAndA" :key="data.question">
        ...
        <!-- no result found -->
        <v-col v-show="!filterFaqData.length" cols="12" class="text-center">
    ...
    <v-card-text class="text-center">
      <p class="text-xl font-weight-medium text--primary mb-1">You still have a question?</p>
...
<style lang="scss">
@import '@resources/sass/preset/pages/faq.scss';
</style>
